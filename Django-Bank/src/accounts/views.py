...
def register_view(request):
    if request.user.is_authenticated:
        ...
        if user_form.is_valid() and account_form.is_valid() and address_form.is_valid():
            ...
            messages.success(
                request,
                '''Merci d'avoir créé un compte trésorerie {}.
                Votre numéro de compte est {}, Veuillez utiliser ce numéro pour vous connecter
                '''.format(new_user.full_name, new_user.account_no))

            return redirect("home")

        context = {
            "title": "Créer un Compte Trésorerie",
            "user_form": user_form,
...
def login_view(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Bienvenu(e), {}!' .format(user.full_name))
            return redirect("home")

        context = {"form": form,
                "title": "Chargement de vos details",
                }
