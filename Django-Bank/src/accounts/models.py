...
class User(AbstractUser):
    username = models.CharField(
        ...
        error_messages={
            'unique': _("un utilisateur avec ce pseudo existe déjà."),
        })
    ...
    grade = models.CharField(max_length=255, unique=True, null=True, blank=True)
    fonction = models.CharField(max_length=255, unique=True, null=True, blank=True)
    ...
    @property
    def amount_arrete(self):
        if hasattr(self, 'account'):
            return self.account.amount_arrete
        return None

    @property
    def amount_percu(self):
        if hasattr(self, 'account'):
            return self.account.amount_percu
        return None

    @property
    def amount_restant(self):
        if hasattr(self, 'account'):
            return self.account.amount_restant
        return None
